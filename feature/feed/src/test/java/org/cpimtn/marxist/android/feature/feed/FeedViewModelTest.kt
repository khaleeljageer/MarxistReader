...
class FeedViewModelTest {
    ...
    fun initial_collection_emits_posts_then_state_is_success() = runTest {
        Dispatchers.setMain(StandardTestDispatcher(testScheduler))
        val posts = listOf(
            Post(1, "2024-01-01", "s", "Title", "Excerpt", emptyList(), emptyList())
        )
        every { getPostsFlowUseCase() } returns flowOf(posts)
        every { getSyncStatusUseCase() } returns flowOf(SyncStatus())
        coEvery { syncPostsUseCase() } returns SyncResult.Success
        ...
        assertEquals(posts, (state as FeedUiState.Success).posts)
    }
